public class Polynome {
    ...
    public double racineParBissectionNIterations(double a, double b, int n) throws NumeriqueException {
        ...
        double fa = this.evalueEn(a);// Calcul de f(a)
        double fb = this.evalueEn(b);// Calcul de f(b)
        if (fa ==0)
            return a;
        if (fb ==0)
            return b;
        ...
        if (fa*fb > 0)
            throw new NumeriqueException("Bolzano non verifie");
        //4) Approximation initiale
        double racine = (a+b)/2;
        //5) Itération
        for (int i = 1 ; i <= n; i++) {
            //a) Vérification que l'approximation n'est pas la racine
            double fracine = this.evalueEn(racine);
            if (fracine ==0) return racine;
    ...
    public double racineParBissection(double a, double b, int d) throws NumeriqueException {
        ...
        int n = (int) Math.ceil(Math.log((Math.pow(10, d))*(b-a))/Math.log(2));
        if (n > MAX_ETAPE)
    ...
    public double racineParNewtonNIterations(double a, double b, int n) throws NumeriqueException {
        ...
        Polynome deriveePremiere = this.polynomeDerive();
        double fpa = deriveePremiere.evalueEn(a);
        double fpb = deriveePremiere.evalueEn(b);
        if (fpa*fpb <=0)
            throw new NumeriqueException("f'(x) pasde signe constant");
        Polynome deriveeSeconde = deriveePremiere.polynomeDerive();
        double fppa = deriveeSeconde.evalueEn(a);
        double fppb = deriveeSeconde.evalueEn(b);
        if (fppa*fppb <=0)
            throw new NumeriqueException("f''(x)pasdesigne constant");
        //4) Approximation initiale
        double racine;
        if (fa * fppa >0) {
            racine = a;
        ...
        for (int i = 1; i <= n; i++) {
            ...
            double fracine = this.evalueEn(racine);
            double fpracine = deriveePremiere.evalueEn(racine);
            racine = racine - (fracine/fpracine);
